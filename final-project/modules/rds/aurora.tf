locals {
  aurora_reader_count = var.use_aurora ? max(var.aurora_instance_count - 1, 0) : 0
}
...
resource "aws_rds_cluster_parameter_group" "aurora" {
  ...
  description = "Parameter group for Aurora cluster ${var.name}"
...
resource "aws_rds_cluster_instance" "aurora_readers" {
  count                = local.aurora_reader_count
